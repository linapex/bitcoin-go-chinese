...
//使用错误的数据模板进行操作码分析的testparseopcode测试。
func TestParseOpcode(t *testing.T) {
//深度复制数组并通过设置使其中一个操作码无效
//长度不对。
...
//testUnparsingInvalidOpcodes在分析无效分析时测试错误
//操作码。
...
//testpusheddata确保pusheddata函数提取预期数据
//各种脚本。
func TestPushedData(t *testing.T) {
	...
	var tests = []struct {
		...
		},
		{
			"16777216 10000000",
			[][]byte{
{0x00, 0x00, 0x00, 0x01}, //一千六百七十七万七千二百一十六
{0x80, 0x96, 0x98, 0x00}, //一千万
			},
			true,
		},
		{
...
//TestHasCanonicalPush确保CanonicalPush函数按预期工作。
...
//testgetprecisesigops确保更精确的签名操作计数
//在p2sh脚本中包含签名的机制按预期工作。
...
//testgetwitnesssigocount测试p2wkh、p2wsh的sig op计数，
//嵌套的p2sh和无效的变量被正确计数。
...
//testremoveopcodes确保从脚本中删除操作码的行为
//预期。
...
//tstreveopcode是一个方便的函数，用于分析提供的
//原始脚本，删除传递的操作码，然后取消对结果的分析
//变成一个原始脚本。
...
//tstreveopcodebydata是一个方便的函数，用于分析提供的
//原始脚本，删除传递的数据，然后取消对结果的分析
//变成一个原始脚本。
...
//testispaytoscripthash确保ispaytoscripthash函数返回
//脚本类测试中所有脚本的预期结果。
...
//testispayToWitnessScriptHash确保isPayToWitnessScriptHash函数
//返回脚本类测试中所有脚本的预期结果。
...
//testispayToWitnessPubKeyHash确保isPayToWitnessPubKeyHash函数
//返回脚本类测试中所有脚本的预期结果。
...
//TestHasCanonicalPush确保CanonicalPush函数正确确定
//对于removeopcodebydata，什么被认为是规范的推送。
...
//testispusOnlyscript确保ispushOnlyscript函数返回
//预期结果。
...
//testinsunspendable确保isUnsendable函数返回预期的
//结果。
