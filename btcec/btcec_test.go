...
//testaddjacobian测试在jacobian坐标中投影的点的加法。
func TestAddJacobian(t *testing.T) {
	tests := []struct {
x1, y1, z1 string //要添加的第一个点的坐标（十六进制）
x2, y2, z2 string //要添加的第二个点的坐标（十六进制）
x3, y3, z3 string //预期点的坐标（十六进制）
	}{
//在无穷远处加上一个点（左手边）。
//Fuff+P= P
		{
...
//testaddaffine测试在仿射坐标中添加点。
func TestAddAffine(t *testing.T) {
	tests := []struct {
x1, y1 string //要添加的第一个点的坐标（十六进制）
x2, y2 string //要添加的第二个点的坐标（十六进制）
x3, y3 string //预期点的坐标（十六进制）
	}{
//在无穷远处加上一个点（左手边）。
//Fuff+P= P
		{
...
//TestDoublejacobian测试Jacobian中投影点的加倍
//协调。
func TestDoubleJacobian(t *testing.T) {
	tests := []struct {
x1, y1, z1 string //点对双的坐标（十六进制）
x3, y3, z3 string //预期点的坐标（十六进制）
	}{
//
		{
...
//testdoubleaffine测试仿射坐标中点的加倍。
func TestDoubleAffine(t *testing.T) {
	tests := []struct {
x1, y1 string //点对双的坐标（十六进制）
x3, y3 string //预期点的坐标（十六进制）
	}{
//在无穷远处加倍一点仍然是无穷大。
//2*∞=∞（无穷远点）
