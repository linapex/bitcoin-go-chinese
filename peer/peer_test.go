...
//conn通过实现net.conn接口模拟网络连接。它
//用于在不实际打开网络的情况下测试对等连接
//连接。
type conn struct {
	io.Reader
	io.Writer
...
//testpeerConnection测试入站和出站对等机之间的连接。
...
ProtocolVersion:   wire.RejectVersion, //使用旧版本配置
		Services:          0,
...
wantBytesSent:       167, //143版+24 verack
...
//testpeerListeners测试是否按预期调用对等侦听器。
...
//testOutboundPeer测试出站对等机是否按预期工作。
...
//testDuplicateVersionMsg确保在一个版本消息之后
//已收到导致对等机断开连接的结果。
